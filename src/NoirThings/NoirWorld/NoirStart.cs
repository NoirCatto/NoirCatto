    ...
    public class NoirStart : UpdatableAndDeletable
    {
        ...
        public Player GetNoirOrNull()
        {
            foreach (var player in room.game.Players)
            {
                ...
                {
                    if (realPlayer.SlugCatClass == Const.NoirName)
                    {
                        return realPlayer;
                    }
                }
        ...
        public override void Update(bool eu)
        {
            if (Cat == null)
            {
                ...
                Cat?.TryGetNoirData(out NoirData);
                return;
            }
            ...
            switch (Phase)
            {
                ...
                case 5:
                    ...
                    if (PhaseCounter >= 30)
                    {
                        UnityEngine.Debug.Log("NoirStart: Failed to hunt Squidcada!");
                        NextPhase();
                    }
                    break;
                case 6:
        ...
        {
            var inpt = new Player.InputPackage(false, Options.ControlSetup.Preset.KeyboardSinglePlayer, 0, 0, false, false, false, false, false);
