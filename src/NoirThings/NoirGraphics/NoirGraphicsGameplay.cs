...
public partial class NoirCatto //Animations + drawing position adjustments are here
{
    ...
    public static bool SlugcatHandOnEngageInMovement(On.SlugcatHand.orig_EngageInMovement orig, SlugcatHand self)
    {
        var player = (Player)self.owner.owner;
        if (player.TryGetNoirData(out var noirData) && noirData.CanCrawlOnBeam())
        {
    ...
    public static void PlayerOnGraphicsModuleUpdated(On.Player.orig_GraphicsModuleUpdated orig, Player self, bool actuallyviewed, bool eu)
    {
