...
public partial class NoirCatto
{
    ...
    public static void PlayerOnUpdateAnimation(On.Player.orig_UpdateAnimation orig, Player self)
    {
        orig(self);
    ...
    public static void PlayerOnMovementUpdate(On.Player.orig_MovementUpdate orig, Player self, bool eu)
    {
        ...
        {
            if (ModOptions.NoirDisableAutoCrouch.Value)
            {
                self.standing = false;
        ...
        if (self.standing)
        {
            if (self.input[0].y == 0 && self.input[0].x != 0 && !ModOptions.NoirDisableAutoCrouch.Value) noirData.StandCounter++;
    ...
    public static void PlayerOnJump(On.Player.orig_Jump orig, Player self)
    {
        if (!self.TryGetNoirData(out var noirData) || ModRotundWorld && IsStuckOrWedged(self))
        {
            orig(self);
            return;
        }
