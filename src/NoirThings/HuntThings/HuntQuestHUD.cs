...
public partial class HuntQuestThings
{
    public class HuntQuestHUD : HudPart //To any poor soul entering here, I apologize in advance. I must uphold the Rain World's poor and uncomprehensible coding standards.
    {
        ...
        {
            foreach (var quest in Master.Quests)
                NewQuest(quest);
        }
                ...
                switch (TutorialMessageShown)
                {
                    ...
                    case 4:
                        if (fade <= minFade)
                        {
                            scriptRunning = false;
                            TutorialMessageShown++;
                            SaveThings.CustomSaveData.SaveStorySpecific(HUNTQUEST_GREETMSG, true, Master.StorySession);
                        }
                        break;
                }
            }
        ...
        {
            SaveThings.CustomSaveData.LoadStorySpecific(HUNTQUEST_GREETMSG, out bool tutorialMessageShown, Master.StorySession);
